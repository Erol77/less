const getData = () => {
    ...
    const getBD = () => {
        fetch('./cross-sell-dbase/dbase.json') //https://jsonplaceholder.typicode.com/todos/
            .then((response) => {
                if (response.ok) {
            ...
            .catch((error) => console.error('false: ', error)); // .finally(()=>{console.log('finally');})
    };
    getBD();
    btn.addEventListener('click',getBD);
};
getData();